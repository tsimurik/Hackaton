...
window.initCity = function() {
  ...
  if (backBtn) {
    ...
    newBackBtn.addEventListener('click', function(e) {
      ...
      // 1. Синхронизируем данные (баланс и здания)
      syncCityDataToMainGame();
      
      // 2. Меняем глобальную переменную режима
      ...
      // 4. Прячем изометрический город
      const mode2Container = document.getElementById('mode2-container');
      if (mode2Container) {
        mode2Container.style.display = 'none';
      }
      
      // 5. Показываем обычную 2D игру
      const mode1Container = document.getElementById('mode1-container');
      if (mode1Container) {
        mode1Container.style.display = 'block';
      }
      
      // 6. Обновляем отображение баланса в 2D игре
      if (typeof window.updateGameBalanceDisplay === 'function') {
        window.updateGameBalanceDisplay();
      }
      if (typeof window.syncBalancesToDom === 'function') {
        window.syncBalancesToDom();
      }
      
      // 7. Перерисовываем 2D сетку
      if (typeof window.renderMinecraftGrid === 'function') {
        window.renderMinecraftGrid();
      }
      
      // 8. Показываем уведомление
      if (typeof window.showGameToast === 'function') {
        window.showGameToast("🏙️ Переключено на Город МТ");
      }
      
      console.log('✅ Возврат в режим 1 завершён');
    });
  }
...
window.syncBalance = function() {
  var user = window.getCurrentUser?.();
  if (user) {
    cityCoins = user.balanceMtBanks || cityCoins;
    updateCoins();
  }
};

window.syncBuildingsFromMain = function(buildingsData) {
  ...
  if (buildingsData) {
    Object.assign(buildings, buildingsData);
  }
...
// Экспортируем функции для доступа из app.js
window.collectAllIncome = collectAll;
window.toggleBuildMode = function() {
  buildModeActive = !buildModeActive;
  var btn = document.getElementById('city-build-btn');
  if (btn) {
    if (buildModeActive) {
      btn.style.background = '#ff9800';
      notify('🏗 Режим строительства включён');
    } else {
      btn.style.background = '#34d399';
      notify('👀 Режим осмотра');
    }
...
// Переопределяем функцию syncBalance чтобы использовать баланс из app.js
var originalSyncBalance = syncBalance;
...
// Переопределяем сохранение чтобы синхронизировать с app.js
var originalSaveBuildings = saveBuildings;
saveBuildings = function() {
  var user = window.getCurrentUser?.();
  if (user) {
    var storageKey = 'mtbank_city_buildings_' + user.id;
    localStorage.setItem(storageKey, JSON.stringify(buildings));
  }
};
// ========== СИНХРОНИЗАЦИЯ С ОСНОВНОЙ ИГРОЙ ==========

// Синхронизация баланса из app.js
window.syncBalanceFromApp = function(amount) {
  cityCoins = amount || 0;
  updateCoins();
  
  // Сохраняем баланс для синхронизации обратно
  localStorage.setItem('city_current_coins', cityCoins);
};

// Синхронизация при сборе монет
var originalCollectAll = collectAll;
collectAll = function() {
  var tot = 0;
  Object.entries(buildings).forEach(([k, b]) => { 
    var a = Math.floor(b.acc); 
    if (a > 0) { 
      b.acc -= a; 
      tot += a; 
    } 
  ...
  if (tot > 0) { 
    ...
    // Сохраняем для синхронизации с app.js
    localStorage.setItem('city_current_coins', cityCoins);
    
    // Синхронизируем с основным аккаунтом
    var user = window.getCurrentUser?.();
    if (user) {
      user.balanceMtBanks = cityCoins;
      var users = window.loadAllUsers?.();
      if (users) {
        users[user.id] = user;
...
// Синхронизация при постройке здания
var originalBuildItem = null;
document.addEventListener('click', function(e) {
  var buildItem = e.target.closest('.build-item');
  if (buildItem && buildItem.onclick) {
    // После постройки синхронизируем
    setTimeout(function() {
      var user = window.getCurrentUser?.();
      if (user) {
        user.balanceMtBanks = cityCoins;
...
window.refreshCityUI = function() {
  updateCoins();
  renderGrid();
  updateCityLevel();
};

// При загрузке города подписываемся на изменения пользователя
window.addEventListener('storage', function(e) {
  if (e.key === 'rr_current_user_id' || e.key === 'rr_registered_users') {
    var user = window.getCurrentUser?.();
    if (user && typeof window.syncBalanceFromApp === 'function') {
      window.syncBalanceFromApp(user.balanceMtBanks || 0);
    }
  }
});

console.log("🔄 Город готов к синхронизации с основным аккаунтом");
// ========== СИНХРОНИЗАЦИЯ ДАННЫХ С ОСНОВНОЙ ИГРОЙ ==========

function syncCityDataToMainGame() {
  console.log("🔄 Синхронизация данных из города в основную игру...");
  ...
  if (typeof window.syncBalanceToMain === 'function') {
    window.syncBalanceToMain(cityCoins);
  } else {
  ...
  // 2. Синхронизируем здания
  syncBuildingsToMainGame();
  
  console.log("✅ Синхронизация завершена. Баланс:", cityCoins);
}

function syncBuildingsToMainGame() {
  var user = window.getCurrentUser?.();
  if (!user) return;
  
  // Получаем здания из города
  var buildingsData = window.buildings || {};
  
  // Конвертируем здания из изометрической версии в основную
  var mainGameBuildings = [];
  for (var i = 0; i < 25; i++) {
    mainGameBuildings.push(null);
  }
  
  // Конвертация ID зданий
  var reverseMapping = {
    'cafe': 'coffee',
    'flower': 'flowershop',
  ...
  for (var key in buildingsData) {
    ...
    if (building && building.id !== 'bank') {
      var coords = key.split(',');
      var row = parseInt(coords[0]);
      var col = parseInt(coords[1]);
      var index = row * 5 + col;
      
      var mainType = reverseMapping[building.id];
      if (mainType && index >= 0 && index < 25) {
        mainGameBuildings[index] = {
          type: mainType,
          level: building.lv || 1,
          pendingIncome: building.acc || 0,
          purchasePrice: 100
        };
      }
    }
  ...
  // Сохраняем в основную игру
  var gameData = {
    buildings: mainGameBuildings,
    lastUpdate: Date.now()
  };
  
  var gameKey = "rr_game_" + user.id;
  localStorage.setItem(gameKey, JSON.stringify(gameData));
  
  console.log("🏢 Синхронизировано зданий в основную игру:", Object.keys(buildingsData).length);
}