(function () {
  ...
  var CATALOG = {
    ...
    brain: {
      title: "Мозги",
      image:
        "https://images.unsplash.com/photo-1606324663549-80d987dfaa8b?w=900&auto=format&fit=crop&q=80",
  ...
  function checkAuth() {
    var userRaw = localStorage.getItem("rr_current_user");
    if (!userRaw) {
  ...
  function init() {
    ...
    var price = randomInt(1200, 48990);
    if (priceDigits) priceDigits.textContent = formatMtbanks(price);

    if (buyBtn && toast) {
      buyBtn.addEventListener("click", function () {
        toast.textContent = "Покупка скоро будет доступна.";
